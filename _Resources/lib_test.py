...
'''
Test suite for libiMobileDevice
    usage: calibre-debug lib_test.py -- [-h] [-b] [-d] [-f] [-i] [-l] [-m] [-v]

Use this code to test changes to the libimobiledevice glue code, which is distributed
as part of the calibre executable. By default, the test suite uses the glue code
from this file instead of the glue code bundled with calibre. Use the '-b' switch
to override this behavior.

'''
import argparse, binascii, cStringIO, inspect, os, plistlib, time, sys, tempfile
...
try:
    from calibre.constants import DEBUG, islinux, isosx, iswindows, is64bit, _plat
...
class Logger():
    ...
    def _log_location(self, *args):
        ...
        print(self.LOCATION_TEMPLATE.format(cls=self.__class__.__name__,
                    func=sys._getframe(1).f_code.co_name,
                    arg1=arg1, arg2=arg2))


class TestSuite(object, Logger):
    '''
    calibre-debug libiMobileDevice_test.py -- -h
    usage: libiMobileDevice_test.py [-h] [-b] [-d] [-f] [-i] [-m] [-v]
    '''
    # class libiMobileDevice() inside
    def __init__(self):
        self._init_parser()
        if self.args.use_bundled_libimobiledevice:
            # Use the shipping version of libiMobileDevice
            from calibre.devices.idevice.libimobiledevice import (
                libiMobileDeviceException, libiMobileDeviceIOException,
                AFC_CLIENT_T, HOUSE_ARREST_CLIENT_T, IDEVICE_T, INSTPROXY_CLIENT_T,
                LOCKDOWND_CLIENT_T, LOCKDOWND_SERVICE_DESCRIPTOR, libiMobileDevice)
            #from calibre.devices.idevice.parse_xml import XmlPropertyListParser
            ...
            # #mark ~~~ Local version of libiMobileDevice glue ~~~
            class libiMobileDevice():
                ...
                def get_preferences(self, requested_items=(
                    ...
                    '''
                    self._log_location()
                    requested_items = self._lockdown_get_value(requested_items)
                ...
                def _lockdown_client_new_with_handshake(self):
                    ...
                    Args:
                     ...
                     client: (LOCKDOWN_CLIENT_T *) The pointer to the location of the new lockdownd client
                ...
                def _lockdown_get_device_name(self):
                    ...
                    #device_name_b = c_char * 128
                    #device_name_p = POINTER(device_name_b)()
                    device_name_p = c_char_p()
                    device_name = None
                    self.lib.lockdownd_get_device_name.restype = c_int
                    ...
                    if error:
                        ...
                        self._log("     device_name: {}".format(device_name))
                    return device_name
        ...
        self.lib = libiMobileDevice(verbose=self.args.verbose)

    def _init_parser(self):
        parser = argparse.ArgumentParser(description="Launch: 'calibre-debug lib_test.py -- <options>'")
        parser.add_argument('-b', '--use_bundled_libimobiledevice', default=False, action='store_true', help='use libiMobileDevice from installed calibre')
        parser.add_argument('-d', '--device_info', default=False, action='store_true', help='display device info')
        parser.add_argument('-f', '--force_error', default=False, action='store_true', help='force file not found error')
        parser.add_argument('-i', '--installed_apps', default=False, action='store_true', help='display installed apps')
        parser.add_argument('-l', '--large_send', default=False, action='store_true', help='send large file to Marvin Library/calibre.mm')
        parser.add_argument('-m', '--marvin', default=False, action='store_true', help='display Marvin /Documents')
        parser.add_argument('-s', '--stats', default=False, action='store_true', help='show stats for Library/calibre.mm')
        parser.add_argument('-v', '--verbose', default=False, action='store_true', help='display libiMobileDevice debug info')
        self.args = parser.parse_args()

    def end_test_suite(self):
        self._log("\n{:*^120}\n".format(" End Test Suite "))

    def evaluate_environment(self):
        self._log("\n{:-^120}".format(" Evaluating environment "))
        env = "OS X"
        this_file = os.path.basename(inspect.stack()[0][1])
        if islinux:
            env = "linux"
        elif iswindows:
            env = "Windows"
        ...
        if self.args.use_bundled_libimobiledevice:
            glue = "using bundled libiMobileDevice glue"
        else:
            glue = "using local libiMobileDevice glue"

        self._log("{0} running under {1} {2}, {3}".format(this_file,
            "64-bit" if is64bit else "32-bit", env, glue))

    def get_connected_devices(self):
        '''
        '''
        return self.lib.get_device_list()

    def run_test_suite(self):
        self._log("\n{:*^120}".format(" libiMobileDevice Test Suite "))
        self.evaluate_environment()

        self.connected_devices = self.get_connected_devices()

        if not self.connected_devices or len(self.connected_devices) > 1:
            if not self.connected_devices:
                self._log("No connected iDevices.")
            elif len(self.connected_devices) > 1:
                self._log("Too many connected iDevices. Connect a single iDevice.")
        else:
            self.show_connected_devices()
            if self.args.device_info:
                self.show_device_info()
            if self.args.marvin:
                self.show_marvin_info()
            if self.args.force_error:
                self.show_forced_error()
            if self.args.installed_apps:
                self.show_installed_apps()
            if self.args.large_send:
    ...
    def show_connected_devices(self):
        ...
        ~~~~~~~~~~ Connected devices ~~~~~~~~~~
        '''
        self._log("\n{:-^120}".format(" Connected devices "))
        self._log(" {0}".format(repr(self.connected_devices)))

    def show_device_info(self):
        '''
        ~~~~~~~~~~ Preferences and device info ~~~~~~~~~~
        '''
        self._log("\n{:-^120}".format(" Device info "))
        connected = self.lib.connect_idevice()
        if not connected:
            self._log("device is locked, unable to get device information")
            return
        preferences = self.lib.get_preferences()
        self.lib.mount_ios_media_folder()
        device_info = self.lib.get_device_info()
        device_info.pop('Model')

        device_profile = dict(preferences.items() + device_info.items())
        for item in sorted(device_profile):
            if item in ['FSFreeBytes', 'FSTotalBytes']:
                self._log(" {0:21}: {1:,}".format(item, int(device_profile[item])))
            else:
                if iswindows:
                    self._log(" {0:21}: {1}".format(item, repr(device_profile[item])))
                else:
                    self._log(" {0:21}: {1}".format(item, device_profile[item]))

        self.lib.dismount_ios_media_folder()
        self.lib.disconnect_idevice()

    def show_forced_error(self):
        '''
        ~~~~~~~~~~ Forced error ~~~~~~~~~~
        Test changes to libiMobileDevice error reporting
        '''
        app_name = "Marvin"
        app_id = "com.appstafarian.MarvinIP"
        self._log("\n{:-^120}".format(" Mounting iOS app '%s' (forced error) " % app_name))
        self.lib.connect_idevice()
        mounted = self.lib.mount_ios_app(app_id=app_id)
        if mounted:
            # Force an error by trying to fetch size of a non-existent epub from Documents
            try:
                foo = self.lib.stat('/'.join(['/Documents', 'absent.epub']))['st_size']
            except:
                import traceback
                formatted_lines = traceback.format_exc().splitlines()
                current_error = formatted_lines[-1]
        else:
            self._log(" '{0}' not installed on this iDevice ".format(app_name))
    ...
    def show_installed_apps(self):
        ...
        ~~~~~~~~~~ Installed apps ~~~~~~~~~~
        '''
        self._log("\n{:-^120}".format(" Installed apps "))
        self.lib.connect_idevice()
        self.lib.get_installed_apps([])
        installed_apps = self.lib.installed_apps
        for ia in installed_apps:
            self._log(" {0:30} {1:50} {2}".format(ia,
                installed_apps[ia]['app_id'],
                installed_apps[ia]['app_version']))

        self.lib.disconnect_idevice()

    def show_marvin_info(self):
        '''
        ~~~~~~~~~~ Marvin Universal ~~~~~~~~~~
        '''
        app_name = "Marvin for iOS"
        app_id = "com.appstafarian.MarvinIP"
        self._log("\n{:-^120}".format(" Mounting iOS app '%s' " % app_name))
        self.lib.connect_idevice()
        mounted = self.lib.mount_ios_app(app_id=app_id)
        #mounted = lib.mount_ios_app(app_name=app_name)
        if mounted:
            files = self.lib.listdir('/Documents')
            self._log("Documents/ ({0} items)".format(len(files)))
    ...
    def test_large_send(self):
        '''
        Test sending very large file to Marvin calibre.mm folder
        '''
        from calibre.ptempfile import TemporaryFile
        app_name = "Marvin for iOS"
        app_id = "com.appstafarian.MarvinIP"
        self._log("\n{:-^120}".format(" Sending large file to '%s' " % app_name))
        self.lib.connect_idevice()
        preferences = self.lib.get_preferences()
        ...
        # Show available, total storage
        self._log("Storage")
        for item in ['FSFreeBytes', 'FSTotalBytes']:
            self._log(" {0:21}: {1:,}".format(item, int(device_profile[item])))
        self._log("{:-^40}".format(''))

        # Show current folder contents
        folder = '/Library/calibre.mm'

        files = self.lib.listdir(folder)
        self._log("{0}/ ({1} items)".format(folder, len(files)))
        for f, d in sorted(files.iteritems()):
            self._log(" {0:30} {1:,}".format(f, int(d['st_size'])))
        self._log("{:-^40}".format(''))

        # Try sending various size files
        test_files = {
                      'test_file.bin': 1234567
                     }

        for fs, size in test_files.iteritems():
            self._log("creating {0} ({1:,})…".format(fs, size))
            with TemporaryFile() as temp:
                with open(temp, 'wb') as f:
                    f.write("x" * size)
                self.lib.copy_to_idevice(temp, '/'.join([folder, fs]))
        self._log("{:-^40}".format(''))

        # Show updated folder contents
        files = self.lib.listdir(folder)
        self._log("{0}/ ({1} items)".format(folder, len(files)))
        for f, d in sorted(files.iteritems()):
            self._log(" {0:30} {1:,}".format(f, int(d['st_size'])))
        ...
        # Delete test files
        self._log("removing test files")
        for fs in test_files:
            #self._log("removing {0}".format(fs))
            self.lib.remove('/'.join([folder, fs]))
        self.lib.disconnect_idevice()

    def test_stats(self):
        '''
        Read stats of a folder
        '''
        from calibre.ptempfile import TemporaryFile
        app_name = "Marvin for iOS"
        app_id = "com.appstafarian.MarvinIP"
        self._log("\n{:-^120}".format(" Reading stats from folder"))
        self.lib.connect_idevice()
        preferences = self.lib.get_preferences()
        ...
        # Show current folder contents
        folder = '/Library/calibre.mm'
        ans = self.lib.stat(folder)
        self._log("stat({0}):".format(folder))
